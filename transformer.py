...
class MultiHeadAttentionSmooth(tf.keras.Model):
    def __init__(self, model_size, n_heads, use_causal_mask):
        super(MultiHeadAttentionSmooth, self).__init__()
        ...
        #norm = 1.0/tf.math.sqrt(tf.dtypes.cast(self.head_size, tf.float32))
        norm = 1.0/np.sqrt(np.array(self.head_size).astype(np.float32))
        
        self.normalizer = self.add_weight('sat_weight', shape=[], initializer=tf.constant_initializer(value=norm), trainable=True)
    ...
    def call(self, query, value, mask=None):
        ...
        #B x T_q x H x d_h
        out_q = tf.reshape(out_q, [out_q.shape[0], out_q.shape[1], self.n_heads, -1])*self.normalizer
        #B x T_k x H x d_h
        ...
        #B x H x T_q x T_k
        score = tf.einsum('bqhd,bkhd->bhqk', out_q, out_k, optimize='optimal')
        if mask is not None:
            score += mask[None,:,:,:]
        
        #maximums = tf.math.reduce_max(score, keepdims=True, axis=-1)
        #the constant is ln(0.5), we remove all attentions that are lower than half of the biggest one
        #score = tf.where(tf.greater(score,maximums-0.693147181), score, -90000000.0) 
...
class GlobalFFN(tf.keras.Model):
    def __init__(self, model_size, n_heads, n_memory):
        super(GlobalFFN, self).__init__()
        self.model_size = model_size
        self.head_size = model_size // n_heads
        self.n_heads = n_heads
        ...
        self.normalizer = 1.0/tf.math.sqrt(tf.dtypes.cast(self.head_size, tf.float32))
        self.w_q = tf.keras.layers.Dense(model_size*4)
        self.wo = tf.keras.layers.Dense(model_size)
        
    def build(self, input_shape):
        self.K = self.add_weight('Kval', shape=[1, self.n_memory, input_shape[2]*4], initializer='glorot_uniform', trainable=True)
        self.V = self.add_weight('Vval', shape=[1, self.n_memory, input_shape[2]], initializer='glorot_uniform', trainable=True)
    ...
    def call(self, query):
        ...
        #B x H x T_q x T_k
        score = tf.einsum('...qhd,...khd->...hqk', out_q, out_k)*self.normalizer

        #B x H x T_q x T_ks
        alignment = tf.nn.softmax(score, axis=-1)
        
        #B x T_q x H x d_h
        heads = tf.einsum('...hqk,...khd->...qhd', alignment, out_v)
...
class TransformerFFNFirst(tf.keras.Model):
    def __init__(self, model_size, num_layers, heads, use_causal_mask, use_counter, normalize_columns, aux_losses=False, abstraction_size=None, token_shift_n=None):
        super(TransformerFFNFirst, self).__init__()
        self.model_size = model_size
        ...
        self.tgt_att = [MultiHeadAttentionSmooth(model_size, heads, use_causal_mask=use_causal_mask) for _ in range(num_layers)]
        self.tgt_att_norm = [normalizer() for _ in range(num_layers)]
        
        self.tgt_ffndense = [tf.keras.layers.Dense(model_size * 11 // 4, activation=tf.math.softplus) for _ in range(num_layers)]
        ...
        self.tgt_alpha = [self.add_weight(f'alpha_tgt{n}', shape=[], initializer=tf.constant_initializer(value=1.0), trainable=True) for n in range(num_layers)]
        self.ffn_alpha = [self.add_weight(f'alpha_ffn{n}', shape=[], initializer=tf.constant_initializer(value=1.0), trainable=True) for n in range(num_layers)]
        self.tgt_beta = [self.add_weight(f'alpha_tgt{n}', shape=[], initializer=tf.constant_initializer(value=1.0), trainable=True) for n in range(num_layers)]
        self.ffn_beta = [self.add_weight(f'alpha_ffn{n}', shape=[], initializer=tf.constant_initializer(value=1.0), trainable=True) for n in range(num_layers)]
        ...
        self.causal_alibi_mask = tf.where(bp==0,-90000000.0,0.0)
    ...
    def call(self, target_sequence, source_sequence=None):
        ...
        for i in range(self.num_layers):
            ...
            tgt_att_out = self.tgt_ffndense[i](tgt_att_out)
...
class Transformer(tf.keras.Model):
    ...
    def call(self, target_sequence, source_sequence=None):
        ...
        for i in range(self.num_layers):
            ...
            #ffn_out = self.global_ffn[i](ffn_mid)
            #ffn_out = self.kms[i](ffn_mid)
            
            ffn_out = self.ffn_beta[i]*ffn_out + self.ffn_alpha[i]*ffn_in #TODO: reverse the order of these things to be the same as with tgt_att_out
