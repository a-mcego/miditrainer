...
class NumpyFileShuffle:
    def get_random_t_data(self, length):
        place = np.random.randint(0,self.tokens_t.shape[0]-length-1, dtype=np.int64)
        return self.tokens_t[place:place+length], self.tokens_t[place+1:place+length+1]
        
    def get_random_t_data_bucket(self, length, bucket_n, total_buckets):
        end = float(self.tokens_t.shape[0]-length-1)
        end = end*bucket_n/total_buckets
